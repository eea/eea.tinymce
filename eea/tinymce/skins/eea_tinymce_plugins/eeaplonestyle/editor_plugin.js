...
(function () {
  tinymce.create("tinymce.plugins.EEAPloneStylePlugin", {
      ...
      if (e) {
        ...
        function ReplaceTag(curelm, newtag) {
          if (curelm.nodeName.toLowerCase() !== newtag) {
            ...
            if (
              (curelm.nodeName.toLowerCase() === "td" ||
                curelm.nodeName.toLowerCase() === "th") &&
              newtag !== "td" &&
              newtag !== "th"
            ) {
              ...
              for (a = 0, length = curelm.attributes.length; a < length; a++) {
                ed.dom.setAttrib(
                  newelm,
                  curelm.attributes[a].name,
                  ed.dom.getAttrib(e, curelm.attributes[a].name)
                );
              }
            } else {
    ...
    _rebuildListBox: function (ed, n) {
      ...
      for (var i = 0; i < this._styles.length; i++) {
        ...
        if (
          ((tag !== "td" && tag !== "th" && tag !== "tr" && tag !== "table") ||
            t) &&
          (tag !== "ul" || ul) &&
          (tag !== "ol" || ol) &&
          ...
          this._control.add(
            ...
            {
              class:
                this._styles[i].className === "-"
                  ? "mceMenuItemTitle"
                  : "mce_formatPreview mce_" + this._styles[i].tag
            }
          );
          var p = this._getParentNode(n, [
            "th",
            "td",
            "p",
            "h1",
          ...
          if (
            p &&
            p.nodeName.toLowerCase() === this._styles[i].tag &&
            p.className.indexOf(this._styles[i].className) !== -1 &&
            il
          ) {
  ...
  tinymce.PluginManager.add(
    "eeaplonestyle",
    tinymce.plugins.EEAPloneStylePlugin
  );
})();
