        ...
        parallel(
          ...
          "i18n": {
            node(label: 'docker-1.13') {
              script {
                try {
                  sh '''docker run -i --net=host --name=$BUILD_TAG-i18n -e EXCLUDE="eea_tinymce_plugins" -e GIT_SRC="https://github.com/eea/$GIT_NAME.git --branch=$BRANCH_NAME" eeacms/i18ndude'''
                } finally {
