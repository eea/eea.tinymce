    ...
    stage('Code') {
      steps {
        parallel(
          ...
          "i18n": {
            node(label: 'docker-1.13') {
              sh '''docker run -i --rm --name=$BUILD_TAG-i18n -e EXCLUDE="eea_tinymce_plugins" -e GIT_SRC="https://github.com/eea/$GIT_NAME.git" -e GIT_NAME="$GIT_NAME" -e GIT_BRANCH="$BRANCH_NAME" -e GIT_CHANGE_ID="$CHANGE_ID" eeacms/i18ndude'''
            }
          }
        )
      }
